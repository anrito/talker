"use client";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { useActionState } from "react";
import { registrationAction } from "@/futures/users/server/actions/AuthentificationActions";

export function RegistrationForm() {
  const [state, registrationFormAction] = useActionState(
    registrationAction,
    undefined
  );

  return (
    <div className="flex items-center justify-center min-h-screen">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle className="text-2xl">შექმენი ანგარიში თოქერზე</CardTitle>
          <CardDescription>შეავსე შენი მონაცემები</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <form action={registrationFormAction}>
            <div className="space-y-2">
              <Label htmlFor="name">სახელი გვარი</Label>
              <Input
                id="name"
                name="name"
                placeholder="შეავსე შენი სახელი და გვარი"
              />
            </div>
            {state?.errors?.name && (
              <p className="text-red-500">{state.errors.name}</p>
            )}
            <div className="space-y-2">
              <Label htmlFor="email">ელ.ფოსტა</Label>
              <Input
                id="email"
                type="email"
                name="email"
                placeholder="შეავსე შენი ელ.ფოსტა"
              />
              {state?.errors?.email && (
                <p className="text-red-500">{state.errors.email}</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="password">პაროლი</Label>
              <Input
                id="password"
                type="password"
                name="password"
                placeholder="შექმენი პაროლი"
              />
              {state?.errors?.password && (
                <p className="text-red-500">{state.errors.password}</p>
              )}
            </div>

            <Button type="submit" className="w-full">
              Register
            </Button>
          </form>
          <div className="relative">
            <div className="absolute inset-0 flex items-center">
              <span className="w-full border-t" />
            </div>
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-2 text-muted-foreground">
                ალტერნატიული რეგისტრაციის მეთოდები
              </span>
            </div>
          </div>
          <Button variant="outline" className="w-full">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24"
              viewBox="0 0 24 24"
              width="24"
              className="mr-2 h-4 w-4"
            >
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
              <path d="M1 1h22v22H1z" fill="none" />
            </svg>
            დარეგისტრირდი Google ით
          </Button>
        </CardContent>
        <CardFooter className="flex justify-center">
          <p className="text-sm text-muted-foreground">
            უკვე გაქვს ექაუნთი?{" "}
            <a href="#" className="text-primary underline">
              ავტორიზაცია
            </a>
          </p>
        </CardFooter>
      </Card>
    </div>
  );
}
